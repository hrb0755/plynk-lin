...
## Overview
This module reads raw input datasets (VCF and phenotype), validates format-level constraints, and emits canonical in-memory/stream records plus metadata for alignment.
...
- Duplicate sample IDs inside phenotype files.
...
## Test Scenarios
- Happy-path VCF + pheno parse with expected row/variant counts.
